...
pclass_survived_mean = titanic.groupby('Pclass')['Survied'].mean().reset_index()
...
plt.xlabel('Age')
plt.ylabel('Fare')
plt.legend(handlse=scatter.legend_elements()[0], title='Survived',
...
plt.title('Corrlation Heatmap of Titanic')
...
# 각 부모와 자녀의 수에 대한 요금의 평균과 표쥰 편차 계산
...
# plt.subplot() 함수를 사용하여 하나의 그림인 figure에 개별 서브플롯을 하니씩 추가
# 첫 번째 서브플롯 설정(선 그래프)
plt.subplot(2, 1, 1)
# 선 그래프 그리기
...
axes[0].set_ylabel('Not Survied Count', color='indigo')
...
# 두 번째 그래프 설정 (막대 그래프)
ax2.bar(x, y2, color='deeppink', aplha=0.7, width=0.7, label='Survived')
ax2.set_ylabel('Survived Count', color='deeppink') # 서브플롯 y축에 레이블 추가
ax2.tick_params(axis='y', labelcolor='deeppink')
ax2.legned(loc='upper right', bbox_to_anchor=(1, 0.9))
